...
# begin properties
properties() { '
kernel.string=San-Kernel-RedSapphire-R1.0.0 by @user_why_red
do.devicecheck=1
do.modules=0
do.systemless=1
do.cleanup=1
...
cpu_ov_level=$(aroma_get_value cpu_ov_level)
cpu_uv_level=$(aroma_get_value cpu_uv_level)
# Read value from aroma end

# Apply uv voltages
if [ "$uv_confirm" -eq 2 ]; then
    ui_print "- Applying UV changes..."
    cpu_uv=$(parse_uv_level $cpu_uv_level)
    [ "$cpu_uv" -ne 0 ] && ${bin}/fdtput $dtb_img /soc/cpr4-ctrl@b018000/thread@0/regulator qcom,custom-voltage-reduce $cpu_uv -tu
    ui_print "- $cpu_uv uV is reduced for all cores!"
    sync
fi
set_progress 0.3
...
if [ "$ov_confirm" -eq 2 ]; then
    ...
    cpu_ov=$(parse_ov_level $cpu_ov_level)
    [ "$cpu_ov" -ne 0 ] && ${bin}/fdtput $dtb_img /soc/cpr4-ctrl@b018000/thread@0/regulator qcom,custom-voltage-increase $cpu_ov -tu
    ui_print "- $cpu_ov uV is increased for all cores!"
    sync
fi
set_progress 0.3
...
if [ "$gpu_oc" -eq 1 ]; then
	...
	ui_print "- GPU is overclocked to 725Mhz(Adreno 506)!"
